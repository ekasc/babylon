...
- [ ] **PARTIAL · 12. Babylon daemon** — transport, persistence, and task ownership are real: framed protocol, `daemon-server.ts` (multi-client, atomic persistence, event broadcast, policy ticks), `daemon-client.ts` (correlated requests, reconnect with backoff, queued calls), `daemon/main.ts` standalone entry, and Electron now acts as a thin client for tasks when `daemon.enabled` is set (spawn + `connectDaemonClient`, `state.get` warm cache, `task.*` through the daemon, `pideck:task-update` from daemon events). Tasks created via `pideck:worktree-create` are `task.created` on the daemon and survive Electron quit via `daemon-state.json` (existing `daemon-server.test.ts` proves create → server restart → still present). `processManager` and `PiHost` are still Electron-owned; closing the app still takes agent work down with it.

  Missing integration: move `PiHost`/session/approval/`LspManager`/`ProcessManager` into the daemon, make all `pideck:*` session/process/approval handlers thin proxies, prove close-and-reopen reconnects to live Pi sessions, and delete the competing in-app sources of truth.
