...
describe("session metadata", () => {
  ...
  it("indexes sessions written flat at the root alongside nested ones", async () => {
    const root = await mkdtemp(join(tmpdir(), "pideck-sessions-flat-"));
    roots.push(root);
    const lines = [
      { type: "session", id: "flat", cwd: "/repo", timestamp: "2026-01-01T00:00:00Z" },
      { type: "message", message: { role: "user", content: "Flat task" } },
    ];
    await writeFile(join(root, "flat.jsonl"), lines.map((line) => JSON.stringify(line)).join("\n") + "\n");
    const index = new SessionIndex(root);
    const groups = await index.list();
    index.dispose();
    expect(groups).toHaveLength(1);
    expect(groups[0]).toMatchObject({ cwd: "/repo" });
    expect(groups[0]?.sessions[0]).toMatchObject({ id: "flat", firstUserText: "Flat task" });
  });
...
describe("tool output clamping", () => {
  it("clamps toolResult text blocks and flags truncation", () => {
    ...
    const content = wireArr(wireOf(message), "content") ?? [];
    const text = content.map((b) => wireStr(wireOf(b), "text") ?? "").join("");
    expect(text.length).toBeLessThanOrEqual(16 * 1024);
    expect(wireOf(message)?.truncated).toBe(true);
  ...
  it("keeps small outputs and diffs untouched", () => {
    ...
    expect(wireOf(small)?.truncated).toBeUndefined();
    const diff = "x".repeat(50_000);
    const withPatch = clampToolOutput({ role: "toolResult", content: [{ type: "text", text: "ok" }], details: { patch: diff } });
    expect(wireStr(wireOf(wireOf(withPatch)?.details), "patch")).toBe(diff); // diffs ship in full
  });
});
...
describe("tail-first session reads", () => {
  it("reads only the tail window of a large file", async () => {
    ...
    expect(tail.messages.map((m) => wireStr(wireOf(m), "content"))).toEqual(["new"]);
  ...
  it("carries the entry timestamp into projected messages so recaps interleave", async () => {
    ...
    expect(typeof wireNum(wireOf(tail.messages[0]), "timestamp")).toBe("number");
    expect(wireNum(wireOf(tail.messages[0]), "timestamp")).toBe(Date.parse("2026-01-01T10:00:00Z"));
  });
  ...
  it("loads older windows ending at a given offset", async () => {
    ...
    expect(older.messages.map((m) => wireStr(wireOf(m), "content"))).toContain("old");
  ...
  it("clamps an oversized end offset to the file size", async () => {
    ...
    expect(result.messages.map((m) => wireStr(wireOf(m), "content"))).toEqual(["safe"]);
  });
