...
export function parseSemver(value: string): ParsedSemver | null {
  ...
  if (majorSegment === undefined || minorSegment === undefined || patchSegment === undefined) return null;
  if (
  ...
  return { major, minor, patch, prerelease: parsePrereleaseSegments(prerelease) };
}
...
export const satisfiesSemverRange: (rawVersion: string, range: string) => boolean =
  function satisfiesSemverRange(rawVersion, range) {
    ...
    const versionMatch = normalizedVersion.match(/^(\d+)(?:\.(\d+))?(?:\.(\d+))?(?:-[0-9A-Za-z.-]+)?$/);
