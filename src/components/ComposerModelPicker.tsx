...
export default function ModelPicker({ models, current, disabled, onSelect }: Props) {
  ...
  return (
    <div ref={rootRef} className="relative">
      ...
      {open && (
        <div className="operator-popover absolute bottom-full left-1/2 z-50 mb-2 w-[460px] max-w-[min(460px,calc(100vw-24px))] -translate-x-1/2 overflow-hidden p-1.5 flex flex-col max-h-[min(420px,calc(100vh-80px))]">
          <div className="border-b border-line/60 p-2">
            <input
              ...
              className="w-full rounded-md border border-line bg-inset px-2.5 py-1.5 text-[12px] placeholder:text-dim outline-none focus:border-accent"
            />
          </div>
          {!searching && tabs.length > 0 ? (
            ...
            <div ref={listRef} role="listbox" aria-label="Search results" className="flex-1 min-h-0 overflow-y-auto py-1">
              {flat.length === 0 && (
