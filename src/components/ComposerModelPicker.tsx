...
export default function ModelPicker({
	...
	const filtered = useMemo(() => {
		...
		const scored = models
			...
			.filter((s) => s.score >= 0);
		scored.sort(
			(a, b) => a.score - b.score || a.m.id.localeCompare(b.m.id),
		);
	...
	const tabs = useMemo<Group[]>(() => {
		...
		if (!searching && recent.length > 0) {
			const rec = recent
				.map((key) =>
					models.find((m) => `${m.provider}/${m.id}` === key),
				)
		...
		for (const [label, ms] of [...byProvider.entries()].sort((a, b) =>
	...
	useLayoutEffect(() => {
		if (!open) return;
		setActiveLabel((prev) => {
			if (!searching && prev && tabs.some((t) => t.label === prev))
				return prev;
	...
	const pick = (provider: string, id: string) => {
		...
		setRecent((prev) => {
			const next = [key, ...prev.filter((k) => k !== key)].slice(
				0,
				MAX_RECENT,
			);
					...
					{!searching && tabs.length > 0 ? (
						<div className="flex overflow-hidden flex-1 min-h-0">
							<div
								...
								className="overflow-y-auto py-1 border-r w-[128px] shrink-0 border-line/60"
							>
								{tabs.map((t) => {
							...
							<div
								ref={listRef}
								role="listbox"
								aria-label={`${activeLabel ?? ""} models`}
								className="overflow-y-auto flex-1 py-1 min-w-0"
							>
									...
									return (
										...
										>
											<span className="flex-1 min-w-0">
												<span
													className={`flex items-center gap-1.5 ${isCurrent ? "font-semibold text-accent" : active ? "text-fg" : "text-fg/85"}`}
												>
						...
						<div
							...
							className="overflow-y-auto flex-1 py-1 min-h-0"
						>
							{flat.length === 0 && (
