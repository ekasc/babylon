...
export default function SettingsModelPicker({ models, current, disabled, onSelect }: Props) {
    ...
    <div ref={rootRef} className="relative">
      ...
      {open && (
        <>
          ...
          <div className="operator-popover absolute top-full right-0 z-50 mt-2 w-[560px] max-w-[min(560px,calc(100vw-24px))] overflow-hidden rounded-xl border border-white/15 bg-[#0F0F0F] shadow-2xl flex flex-col max-h-[min(560px,calc(100vh-80px))] p-1.5">
            <div className="pointer-events-none absolute -top-1.5 right-[140px] h-3 w-3 rotate-45 border-l border-t border-white/15 bg-[#0F0F0F]" aria-hidden />
          ...
          {!searching && tabs.length > 0 ? (
            <div className="flex flex-1 min-h-0">
              <div role="tablist" aria-label="Providers" className="w-[150px] shrink-0 overflow-y-auto border-r border-white/10 bg-white/[0.02] py-2">
                {tabs.map((t) => {
