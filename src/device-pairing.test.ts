...
describe("device pairing with tokens", () => {
  ...
  it("refuses empty scope, unknown scope, blank fields, and duplicate ids", () => {
    ...
    // Intentionally invalid scope: the pairing validator must reject it.
    expect(pairDevice(registry, { ...base, scope: ["root_everything" as DeviceScope] })).toMatch(/unknown device scope/);
  ...
  it("verifies tokens and rejects revoked or unhashed devices", () => {
    ...
    if (!device?.tokenHash) throw new Error("missing device");
    ...
    const revokedHash = revoked.devices.d1?.tokenHash;
    if (!revokedHash) throw new Error("missing device");
    expect(verifyToken("secret", revokedHash)).toBe(true); // hash still matches...
  ...
  it("keeps listDevices copies free of mutation side effects", () => {
    ...
    if (!firstListed) throw new Error("missing device");
    firstListed.scope.push("approve_deny");
    firstListed.revoked = true;
    expect(registry.devices.d1?.scope).toEqual(["view_tasks"]);
